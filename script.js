...
const state = {
  ...
  adminGrupos: [],
  adminGrupoUsuarios: [],
  adminGrupoParticipantesSelecionados: [],
...
const els = {
  ...
  btnAtualizarAdminGrupos: $("btnAtualizarAdminGrupos"),
  adminGruposLista: $("adminGruposLista"),
  adminGrupoSelecionadoInfo: $("adminGrupoSelecionadoInfo"),
  adminGrupoId: $("adminGrupoId"),
  adminGrupoUsuariosLista: $("adminGrupoUsuariosLista"),
  adminGrupoSelecionadosInfo: $("adminGrupoSelecionadosInfo"),
  btnSalvarAdminGrupoParticipantes: $("btnSalvarAdminGrupoParticipantes"),
  btnExcluirGrupoAdmin: $("btnExcluirGrupoAdmin"),
  btnLimparAdminGrupo: $("btnLimparAdminGrupo"),
...
function configurarEventos() {
  ...
  els.adminTabGrupos.addEventListener("click", () => mostrarAbaAdmin("grupos"));
  ...
  els.btnAtualizarAdminGrupos.addEventListener("click", carregarAdminGrupos);
  els.btnSalvarAdminGrupoParticipantes.addEventListener("click", salvarParticipantesGrupoAdmin);
  els.btnExcluirGrupoAdmin.addEventListener("click", excluirGrupoAdmin);
  els.btnLimparAdminGrupo.addEventListener("click", limparFormularioAdminGrupo);
...
async function carregarUsuarios() {
  const { data, error } = await supabaseClient
    .from("perfis")
    .select("id, nome, email, cargo, status, ativo, is_admin, setor_id, excluido, setores(nome)")
...
function limparEstado() {
  ...
  state.adminUsuarios = [];
  state.adminGrupos = [];
  state.adminGrupoUsuarios = [];
  state.adminGrupoParticipantesSelecionados = [];
...
function mostrarAbaAdmin(aba) {
  ...
  const grupos = aba === "grupos";

  els.adminTabUsuarios.classList.toggle("active", usuarios);
  els.adminTabSetores.classList.toggle("active", setores);
  els.adminTabGrupos.classList.toggle("active", grupos);

  els.adminUsuariosPainel.classList.toggle("hidden", !usuarios);
  els.adminSetoresPainel.classList.toggle("hidden", !setores);
  els.adminGruposPainel.classList.toggle("hidden", !grupos);
...
async function carregarAdminUsuarios() {
  const { data, error } = await supabaseClient
    .from("perfis")
    .select("id, nome, email, cargo, setor_id, is_admin, ativo, status, excluido, setores(nome)")
    .or("excluido.is.null,excluido.eq.false")
    .order("nome", { ascending: true });
...
function renderizarAdminUsuarios() {
        ...
        <tr class="${classeLinha}">
          ...
          <td>
            <div class="table-actions">
              <button class="mini-btn icon-btn" type="button" data-admin-user-id="${u.id}" title="Editar usuário" aria-label="Editar usuário">
                ✏️
              </button>
              <button class="mini-btn icon-btn ${classeBotaoStatus}" type="button" data-toggle-user-id="${u.id}" data-novo-status="${novoStatus}" title="${textoBotaoStatus} usuário" aria-label="${textoBotaoStatus} usuário">
                ${u.ativo ? "🚫" : "✅"}
              </button>
              <button class="mini-btn icon-btn danger-mini" type="button" data-delete-user-id="${u.id}" title="Excluir usuário" aria-label="Excluir usuário">
                🗑️
              </button>
            </div>
          </td>
  ...
  document.querySelectorAll("[data-delete-user-id]").forEach((btn) => {
    btn.addEventListener("click", () => excluirUsuarioSistema(btn.dataset.deleteUserId));
  });
}
...
async function excluirUsuarioSistema(usuarioId) {
  ...
  const confirmar = window.confirm(
    `Deseja excluir ${nome} do sistema?\n\n` +
    "Ele será removido dos grupos, ficará bloqueado e não aparecerá mais nas listas."
  );

  if (!confirmar) return;

  const { error } = await supabaseClient.rpc("admin_excluir_usuario_sistema", {
    p_usuario_id: usuarioId
  });
  ...
  if (state.usuarioSelecionado?.id === usuarioId) {
    limparConversaAtual("Usuário excluído do sistema.");
  }

  limparFormularioAdminUsuario();
  await sincronizarGruposSilencioso();
  await carregarAdminUsuarios();
  await atualizarConversasLaterais();
  await carregarAdminGrupos();

  toast("Usuário excluído do sistema.");
}

async function carregarAdminGrupos() {
  if (!state.perfil?.is_admin || !els.adminGruposLista) return;

  const { data, error } = await supabaseClient.rpc("admin_listar_grupos");

  if (error) {
    els.adminGruposLista.innerHTML = `<div class="empty">Erro ao carregar grupos: ${escapeHtml(error.message)}</div>`;
    return;
  }

  state.adminGrupos = data || [];
  renderizarAdminGrupos();
}

function renderizarAdminGrupos() {
  if (!els.adminGruposLista) return;

  if (!state.adminGrupos.length) {
    els.adminGruposLista.innerHTML = `<div class="empty">Nenhum grupo encontrado.</div>`;
    return;
  }

  els.adminGruposLista.innerHTML = state.adminGrupos
    .map((g) => {
      const tipo = g.tipo_grupo === "setor" ? "Setor" : "Personalizado";
      const setor = g.setor_nome ? ` • ${escapeHtml(g.setor_nome)}` : "";
      const participantes = Number(g.participantes || 0);
      const ativo = els.adminGrupoId?.value === g.conversa_id ? "active" : "";

      return `
        <div class="admin-group-item ${ativo}" data-admin-group-card="${g.conversa_id}">
          <div>
            <strong>${escapeHtml(g.nome || "Grupo")}</strong>
            <span>${tipo}${setor} • ${participantes} participante(s)</span>
          </div>
          <div class="table-actions">
            <button class="mini-btn icon-btn" type="button" data-manage-group-id="${g.conversa_id}" title="Gerenciar participantes" aria-label="Gerenciar participantes">
              👥
            </button>
            <button class="mini-btn icon-btn danger-mini" type="button" data-delete-group-id="${g.conversa_id}" title="Excluir grupo" aria-label="Excluir grupo">
              🗑️
            </button>
          </div>
        </div>
  ...
  document.querySelectorAll("[data-manage-group-id]").forEach((btn) => {
    btn.addEventListener("click", () => selecionarAdminGrupo(btn.dataset.manageGroupId));
  });

  document.querySelectorAll("[data-delete-group-id]").forEach((btn) => {
    btn.addEventListener("click", () => excluirGrupoAdmin(btn.dataset.deleteGroupId));
  });
}

async function selecionarAdminGrupo(conversaId) {
  const grupo = state.adminGrupos.find((g) => g.conversa_id === conversaId);
  ...
  els.adminGrupoId.value = conversaId;
  els.adminGrupoSelecionadoInfo.textContent =
    `${grupo.nome || "Grupo"} • ${grupo.tipo_grupo === "setor" ? "Grupo por setor" : "Grupo personalizado"}`;

  els.btnSalvarAdminGrupoParticipantes.disabled = false;
  els.btnExcluirGrupoAdmin.disabled = false;

  await carregarParticipantesGrupoAdmin(conversaId);
  renderizarAdminGrupos();
}

async function carregarParticipantesGrupoAdmin(conversaId) {
  els.adminGrupoUsuariosLista.innerHTML = `<div class="empty">Carregando participantes...</div>`;

  const { data, error } = await supabaseClient.rpc("admin_listar_participantes_grupo", {
    p_conversa_id: conversaId
  });

  if (error) {
    els.adminGrupoUsuariosLista.innerHTML = `<div class="empty">Erro ao carregar participantes: ${escapeHtml(error.message)}</div>`;
    return;
  }

  state.adminGrupoUsuarios = data || [];
  state.adminGrupoParticipantesSelecionados = state.adminGrupoUsuarios
    .filter((u) => u.participante)
    .map((u) => u.usuario_id);

  renderizarParticipantesGrupoAdmin();
}

function renderizarParticipantesGrupoAdmin() {
  if (!state.adminGrupoUsuarios.length) {
    els.adminGrupoUsuariosLista.innerHTML = `<div class="empty">Nenhum usuário ativo disponível.</div>`;
    els.adminGrupoSelecionadosInfo.textContent = "0 participante(s) selecionado(s)";
    return;
  }

  els.adminGrupoSelecionadosInfo.textContent =
    `${state.adminGrupoParticipantesSelecionados.length} participante(s) selecionado(s)`;

  els.adminGrupoUsuariosLista.innerHTML = state.adminGrupoUsuarios
    .map((u) => {
      const checked = state.adminGrupoParticipantesSelecionados.includes(u.usuario_id) ? "checked" : "";
      const inicial = pegarInicial(u.nome || u.email);

      return `
        <label class="grupo-user-item">
          <input type="checkbox" value="${u.usuario_id}" ${checked} />
          <div class="avatar small-avatar">${inicial}</div>
          <div class="user-info">
            <strong>${escapeHtml(u.nome || "Sem nome")}</strong>
            <span>${escapeHtml(u.cargo || "Usuário")} • ${escapeHtml(u.setor_nome || "Sem setor")}</span>
          </div>
        </label>
      `;
    })
  ...
  els.adminGrupoUsuariosLista.querySelectorAll('input[type="checkbox"]').forEach((check) => {
    check.addEventListener("change", () => {
      const usuarioId = check.value;

      if (check.checked && !state.adminGrupoParticipantesSelecionados.includes(usuarioId)) {
        state.adminGrupoParticipantesSelecionados.push(usuarioId);
      }

      if (!check.checked) {
        state.adminGrupoParticipantesSelecionados =
          state.adminGrupoParticipantesSelecionados.filter((id) => id !== usuarioId);
      }
...
async function salvarParticipantesGrupoAdmin() {
  const conversaId = els.adminGrupoId.value;
  ...
  if (!state.adminGrupoParticipantesSelecionados.length) {
    toast("Selecione pelo menos 1 participante para o grupo.");
    return;
  }

  const { error } = await supabaseClient.rpc("admin_atualizar_participantes_grupo", {
    p_conversa_id: conversaId,
    p_participantes: state.adminGrupoParticipantesSelecionados
  });
  ...
  const grupoAtual = state.adminGrupos.find((g) => g.conversa_id === conversaId);
  if (grupoAtual) {
    await selecionarAdminGrupo(conversaId);
  }

  toast("Participantes do grupo atualizados.");
}

async function excluirGrupoAdmin(conversaIdParametro = null) {
  const conversaId = conversaIdParametro || els.adminGrupoId.value;
  ...
  const confirmar = window.confirm(
    `Deseja excluir o grupo "${nome}"?\n\n` +
    "As mensagens e participantes desse grupo também serão removidos."
  );

  if (!confirmar) return;

  const { error } = await supabaseClient.rpc("admin_excluir_grupo", {
    p_conversa_id: conversaId
  });

  if (error) {
    toast("Erro ao excluir grupo: " + error.message);
    return;
  }

  if (state.conversaAtual?.id === conversaId) {
    limparConversaAtual("Grupo excluído do sistema.");
  }

  limparFormularioAdminGrupo();
  await carregarAdminGrupos();
  await carregarGrupos();
  await carregarSetores();
...
function limparFormularioAdminGrupo() {
  els.adminGrupoId.value = "";
  els.adminGrupoSelecionadoInfo.textContent = "Selecione um grupo na lista.";
  els.adminGrupoUsuariosLista.innerHTML = `<div class="empty">Selecione um grupo para editar os participantes.</div>`;
  els.adminGrupoSelecionadosInfo.textContent = "0 participante(s) selecionado(s)";
  els.btnSalvarAdminGrupoParticipantes.disabled = true;
  els.btnExcluirGrupoAdmin.disabled = true;
  state.adminGrupoUsuarios = [];
  state.adminGrupoParticipantesSelecionados = [];
  renderizarAdminGrupos();
}

function limparConversaAtual(mensagem = "Selecione uma conversa.") {
  state.usuarioSelecionado = null;
  state.conversaAtual = null;
  state.conversaTipo = null;
  state.participantesAtual = [];
  ...
  els.chatTitulo.textContent = "Selecione um usuário";
  els.chatSubtitulo.textContent = mensagem;
  els.mensagensLista.innerHTML = `
    <div class="welcome">
      <h3>${escapeHtml(mensagem)}</h3>
      <p>Escolha uma conversa ou grupo para continuar.</p>
    </div>
  `;
...
function renderizarSetoresAdmin() {
      ...
      return `
        <div class="setor-item setor-item-detalhado">
          ...
          <div class="setor-actions">
            ${s.conversa_id ? `<button class="mini-btn" type="button" data-gerenciar-grupo-id="${s.conversa_id}">Participantes</button>` : ""}
  ...
  document.querySelectorAll("[data-gerenciar-grupo-id]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      mostrarAbaAdmin("grupos");
      await carregarAdminGrupos();
      await selecionarAdminGrupo(btn.dataset.gerenciarGrupoId);
    });
  });
